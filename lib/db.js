...
export async function dbAddBook(userId, book) {
  ...
  if (book.isbn) row.isbn = book.isbn;
  ...
  // If isbn column doesn't exist yet, retry without it
  if (error && (error.message || "").includes("isbn")) {
    delete row.isbn;
    const retry = await supabase.from("user_collection").insert(row).select().single();
    data = retry.data;
    error = retry.error;
  }
...
export async function dbUpdateBook(bookId, book, userId) {
  ...
  if (book.isbn) updates.isbn = book.isbn;
  ...
  if (error && (error.message || "").includes("isbn")) {
    delete updates.isbn;
    const retry = await supabase.from("user_collection").update(updates).eq("id", bookId);
    error = retry.error;
  }
