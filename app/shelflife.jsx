...
import {
  buildMarketPricePoints, calculateMarketStats, calculateConfidence,
} from "../lib/pricing";
import {
  dbLoadCollection, dbAddBook, dbUpdateBook, dbDeleteBook,
  dbLoadWishlist, dbAddWishlistItem, dbDeleteWishlistItem,
  dbSendContactMessage, dbLoadProfile, dbUpdateProfile,
  dbGetCommunityActivity, dbGetPublicCollectors, dbLoadPublicCollectorProfile,
  dbGetRecentPriceReports, dbCheckDuplicate, dbGetCollectionValues,
  dbGetRecentCollectionEntries, dbReportSale, dbGetPriceReports,
...
/* Database functions are in ../lib/db.js */
...
function BookForm({ book, onSave, onCancel, isEdit, userId }) {
  ...
  return (<div>
    ...
    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
      ...
      <div><label style={labelBase}>ISBN</label><input style={inputBase} value={f.isbn||""} onChange={e=>s("isbn",e.target.value)} placeholder="978-0-000-00000-0" /></div>
...
function DetailView({ book, onEdit, onDelete, onClose, onUpdateCover, user }) {
  ...
  return (<div>
    ...
    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:14 }}>{[["Publisher",book.publisher],["Edition",book.editionType],["Limitation",book.limitation],["Condition",book.condition],["ISBN",book.isbn],["Added",book.dateAdded]].map(([l,v])=>v?<div key={l}><div style={labelBase}>{l}</div><div style={{ color:"#e0d6c8", fontSize:13 }}>{v}</div></div>:null)}</div>
